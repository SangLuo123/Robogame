...
| ├── new_main3.py          --理想的比赛全流程
| ├── new_main4.py          --实际比赛用到的函数，拿三个常规飞镖进行打哨所
| ├── new_main7.py          --实际比赛中用到的函数，与上个不同，该函数拿到第一个飞镖直接发射不管方向，先拿到2分
| ├── one_cam_tag.py        --一个相机测试主函数
| ├── photo.py              --拍照
| ├── read_file             --读取配置（异常处理时需要考虑场上飞镖数进行决策，虽然也没用到）
| ├── sim_main.py           --调用一个摄像头实时扫描tag
| ├── sim_main1.py          --调用两个摄像头实时扫描tag
