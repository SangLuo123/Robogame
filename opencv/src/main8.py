...
def find_dart_1(frame, roi1, roi2, roi3, hsv, min_area):
    """
    检测三个ROI区域中是否有飞镖，返回第一个有飞镖的ROI序数
    
    参数:
    frame: 输入图像帧
    roi1, roi2, roi3: 三个ROI区域，格式为 (x, y, width, height)
    hsv: HSV阈值范围，格式为 ((h_low, s_low, v_low), (h_high, s_high, v_high))
    min_area: 最小面积阈值，用于判断是否有飞镖
    
    返回:
    int: 第一个有飞镖的ROI序数 (1, 2, 或 3)，如果没有找到则返回None
    """
    ...
    # 创建HSV掩码
    lower_bound = np.array(hsv[0])
    upper_bound = np.array(hsv[1])
    ...
    for i, roi in enumerate(rois, 1):
        ...
        # 计算ROI区域中符合HSV阈值的像素数量
        target_pixels = np.count_nonzero(roi_mask)
        roi_area = w * h
        
        # 计算目标像素占比
        target_ratio = target_pixels / roi_area if roi_area > 0 else 0
        
        # 如果目标像素占比超过最小面积阈值，则认为有飞镖
        print(f"target_ratio: {target_ratio}, min_area: {min_area}")
        if target_ratio >= min_area:
            return i  # 返回ROI序数 (1, 2, 3)
    
    # 如果没有找到符合条件的ROI（虽然你说至少有一个，但为了代码健壮性）
    return None
...
def main():
    ...
    print("[RESULT]", res)
    frame = mc.latest("cam1").image
    roi1 = (0, 0, 0, 0)
    roi2 = (0, 0, 0, 0)
    roi3 = (0, 0, 0, 0)
    hsv = ((0, 0, 0), (180, 255, 255))
    index = find_dart_1(frame, roi1, roi2, roi3, hsv, 50)
    dx = 0
    dy = 0
    ok, info = _send_move_and_wait_ack(link, d_x, 0)
    if not ok:
